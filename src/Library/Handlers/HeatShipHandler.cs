...
namespace Library
{
    ...
    /// </summary>
    public class HeatShipHandler : BaseHandler
    {
        /// <summary>
        /// Inicializa una nueva instancia de la clase <see cref="UserStatsHandler"/>. Esta clase procesa el mensaje "/Estadisticas".
        /// </summary>
        /// <param name="next">El próximo "handler".</param>
        public HeatShipHandler(BaseHandler next) : base(next)
        {
            this.Keywords = new string[] { "/Disparos_Barcos" };
        }
        ...
        protected override bool InternalHandle(Message message, out string response)
        {
            if (this.CanHandle(message) && MatchList.Instance.FindMatch(message.From.ToString())!=null)
            {
                ...
                completeMessage.Append($"Los disparos efectuados a barcos son: {match.Battle.ShipHeat}");
